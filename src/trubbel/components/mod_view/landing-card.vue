<template>
  <a class="ffz-mod-landing-card" :class="isLive ? 'ffz-mod-landing-card--live' : 'ffz-mod-landing-card--offline'"
    :href="'/moderator/' + node.login" :style="'--ffz-mod-accent: #' + (node.primaryColorHex || 'a970ff') + ';'"
    @click="onClick">

    <img class="ffz-mod-landing-avatar" :src="node.profileImageURL" :alt="node.displayName" width="44" height="44">

    <div class="ffz-mod-landing-info">
      <div class="ffz-mod-landing-name-row">

        <span class="ffz-mod-landing-name" :style="node.chatColor ? 'color:' + node.chatColor : ''">{{ nameDisplay
        }}</span>

        <span v-if="isEditor"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--editor"
          data-tooltip-type="text" data-title="Editor">{{ t('trubbel.mod-landing.badge.editor', 'editor') }}</span>

        <span v-if="isLeadMod"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--lead"
          ...
          <svg width="12" height="12" viewBox="0 0 512 512" fill="currentColor" aria-hidden="true"
            class="ffz-mod-landing-sub-icon">
            <path
        ...
        <span v-if="isFounder"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--founder"
          data-tooltip-type="text" data-title="Founder">{{ t('trubbel.mod-landing.badge.founder', 'founder') }}</span>

        <span v-if="showSubBadge"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--sub"
          :class="{ 'ffz-mod-landing-badge--sub-lapsed': !benefit }" data-tooltip-type="text" :data-title="subTooltip">

          <svg v-if="subIconKey === 'hollow-star'" width="12" height="12" viewBox="0 0 24 24" fill="currentColor"
            aria-hidden="true" class="ffz-mod-landing-sub-icon">
        ...
        <span v-if="giftCount > 0"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--gifts"
        ...
        <span v-if="isFirstTimeChatter"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-badge ffz-mod-landing-badge--ftc"
      ...
      <div v-if="title" class="ffz-mod-landing-title">{{ title }}</div>

      <div v-if="game && game.displayName" class="ffz-mod-landing-game-row">
        <img v-if="game.boxArtURL" class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-game-thumb"
        ...
        <span class="ffz-mod-landing-game">{{ game.displayName }}</span>
      </div>
    </div>

    <div class="ffz-mod-landing-right">
      <template v-if="isLive">
        <div class="ffz-mod-landing-viewers">
          <span class="ffz-mod-landing-viewers-dot"></span>
          {{ formatViewers(node.stream && node.stream.viewersCount) }}
        </div>
        <div v-if="node.stream && node.stream.createdAt"
          class="ffz-tooltip ffz-tooltip--no-mouse ffz-mod-landing-uptime" data-tooltip-type="text"
      ...
      <template v-else>
        <div class="ffz-mod-landing-last-live" :class="{ 'ffz-tooltip ffz-tooltip--no-mouse': lastLiveDate }"
